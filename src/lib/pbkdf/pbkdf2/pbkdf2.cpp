...
size_t tune_pbkdf2(MessageAuthenticationCode& prf,
   ...
   const size_t trials = (msec > 1000 ? 64 : 4);
   ...
   const uint64_t duration_nsec = (end_nsec - start_nsec) / trials;
...
std::unique_ptr<PasswordHash> PBKDF2_Family::tune(size_t output_len, std::chrono::milliseconds msec) const
   {
