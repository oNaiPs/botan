...
std::unique_ptr<PasswordHash> RFC4880_S2K_Family::tune(size_t output_len, std::chrono::milliseconds msec) const
   {
...
std::unique_ptr<PasswordHash> RFC4880_S2K_Family::default_params() const
   {
   return std::unique_ptr<PasswordHash>(new RFC4880_S2K(m_hash->clone(), 255));
   }
...
RFC4880_S2K::RFC4880_S2K(HashFunction* hash, size_t iterations) :
   ...
   if(iterations < 256)
      ...
      size_t rounded = RFC4880_round_iterations(iterations);
      if(rounded != iterations)
         printf("bad puppy rt iter error %d %d\n", rounded, iterations);
      }
   }
